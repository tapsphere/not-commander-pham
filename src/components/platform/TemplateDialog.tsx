...
export const TemplateDialog = ({ open, onOpenChange, template, onSuccess, onTemplateCreated, demoMode = false }: TemplateDialogProps) => {
  ...
  useEffect(() => {
    const fetchCompetencies = async () => {
      try {
        const response = await apiClient.get('/api/framework/competencies?is_active=eq.true');
  ...
  useEffect(() => {
    ...
    const fetchSubCompetencies = async () => {
      try {
        const response = await apiClient.get(`/api/framework/sub-competencies?competency_id=eq.${selectedCompetency}`);
        const sortedData = (response.data || []).sort((a: any, b: any) => (a.display_order || 0) - (b.display_order || 0));
        setSubCompetencies(sortedData);
      } catch (error) {
  ...
  useEffect(() => {
    if (selectedSubCompetencies.length > 0 && subCompetencies.length > 0) {
      const loadSampleAuto = async () => {
        ...
        let subCompData = null;
        try {
          const response = await apiClient.get(`/api/framework/sub-competencies?id=eq.${selectedSub.id}`);
          subCompData = response.data?.[0];
        } catch (subError) {
        ...
        if (!subCompData) {
          console.error('Sub-competency not found');
          return;
        }
  ...
  const handleValidateCustomGame = async () => {
    ...
    try {
      ...
      if (isCustomUpload && customGameFile) {
        ...
        uploadFormData.append('file', customGameFile);

        const uploadRes = await apiClient.post('/api/storage/upload', uploadFormData, {
        ...
        const draftRes = await apiClient.post('/api/templates', {
        ...
        draftTemplateId = draftRes.data.id;
      } else {
  ...
  <div class="container">
    ...
    <div class="actions">
      ...
      if (selectedSubCompetencies.length > 0) {
        try {
          const response = await apiClient.get(`/api/framework/sub-competencies?id=in.(${selectedSubCompetencies.join(',')})`);
          if (response.data) {
            subCompetenciesData = response.data;
          }
        } catch (e) {
          console.error(e);
        }
  ...
  const handleSubmit = async (e: React.FormEvent, saveAsDraft: boolean = false) => {
    ...
    try {
      ...
      if (customGameFile) {
        ...
        fileFormData.append('file', customGameFile);

        const uploadRes = await apiClient.post('/api/storage/upload', fileFormData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });

        customGameUrl = uploadRes.data.url;
      }
      ...
      if (coverImageFile) {
        ...
        const uploadRes = await apiClient.post('/api/storage/upload', coverFormData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        ...
        let profile = null;
        try {
          const profileRes = await apiClient.get('/api/profiles/me');
        ...
        if (profile) {
          ...
          const fileName = `default-cover-${Date.now()}.png`;
          const defaultCoverForm = new FormData();
          defaultCoverForm.append('file', coverBlob, fileName);

          try {
            const uploadRes = await apiClient.post('/api/storage/upload', defaultCoverForm, {
              headers: { 'Content-Type': 'multipart/form-data' }
            });
            coverImageUrl = uploadRes.data.url;
          } catch (uploadError) {
      ...
      if (template?.id) {
        ...
        // Create new template
        const newTemplateRes = await apiClient.post('/api/templates', {
          name: formData.name,
          description: formData.description,
      ...
      {draftTemplateId && selectedSubCompetencies[0] && (
        <ValidatorTestWizard
          ...
          onComplete={() => {
            ...
            apiClient.get(`/api/games/validator-test-results?template_id=${draftTemplateId}`)
              .then((response: any) => {
                const data = response.data?.[0]; // simplified assume first is latest
