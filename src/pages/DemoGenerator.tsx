...
export default function DemoGenerator() {
  ...
  const ensureDemoTemplateUploaded = async () => {
    try {
      console.log('Updating demo template in storage...');

      // Always fetch and upload the latest template
      ...
      const file = new File([blob], 'demo-crisis-template.html', { type: 'text/html' });
      const formData = new FormData();
      formData.append('file', file);
      formData.append('path', 'demo-crisis-template.html');

      await apiClient.post('/storage/upload/custom-games', formData);

      console.log('Demo template updated successfully');
      toast({
        title: "Template Updated",
        description: "Latest demo template is ready",
      });
    } catch (error) {
      console.error('Error updating demo template:', error);
      toast({
    ...
    try {
      ...
      if (data) {
        setBrandProfile({
          ...
          game_avatar_url: data.game_avatar_url,
          primary_color: data.design_palette?.primary,
          secondary_color: data.design_palette?.secondary,
        });
      }
    } catch (error) {
  ...
  const generateGame = async () => {
    ...
    try {
      const prompt = `Course Name: ${extractedData.course_name || "Leadership Training"}. Brand: ${brandProfile?.company_name || extractedData.company_name || "Your Company"}. Description: ${extractedData.course_description || ""}. Objectives: ${(extractedData.learning_objectives || []).join(', ')}`;
      const { data: response } = await apiClient.post("/games/generate", {
        ...
        avatarUrl: brandProfile?.game_avatar_url || null,
        previewMode: true
      });

      // Create a blob URL for the generated HTML
      const html = response.generatedHtml || response.html || "<h1>Failed to generate game content</h1>";
